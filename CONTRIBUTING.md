...
Since the Superuser UI is just a client-side application, you need to have the PocketBase backend server also running in the background (either manually or via `go run main.go serve` from `examples/base`).

> **Personal note:** I run the backend with `--dev` flag during UI development (`go run main.go serve --dev`) to get more verbose logging, which is helpful for debugging API calls from the frontend.
